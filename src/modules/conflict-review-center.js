...
export class ConflictReviewCenter{
  ...
  async init(){if(this.initialized)return;this.initialized=true;
    ...
    eventBus.on('conflict:false-positive-resolved',()=>this.refresh());
    await this.refresh();
  }
  async refresh(){await conflictResolutionManager.cleanupFalsePositiveConflicts();this.rows=(await conflictRepository.all()).sort((a,b)=>String(b.detectedAt||'').localeCompare(String(a.detectedAt||'')));this.syncDomainFilter();this.render()}
