...
export class SyncFoundationUI{
  ...
  async refreshBaseline(){const x=await cloudFoundationCleanupService.status();const b=x.baseline;this.$('baselineSamples').textContent=String(x.sampleCount);this.$('baselineOutbox').textContent=String(x.outboxPending);this.$('baselineAudit').textContent=String(x.auditCount);this.$('baselineStatus').textContent=b?.status==='READY_FOR_INITIAL_CLOUD_SEED'?'READY':'PENDIENTE';this.$('baselineInfo').innerHTML=b?`<div class="notice"><b>✅ Baseline creado:</b> ${esc(b.baselineId)} · ${new Date(b.createdAt).toLocaleString('es-EC')} · Outbox histórico archivado/limpiado: ${b.outboxArchived}.</div>`:`<div class="notice warn"><b>Baseline pendiente.</b> Antes de Firebase conviene archivar y vaciar el Outbox histórico, conservar Audit y marcar la fotografía local actual como READY.</div>`;return x}
